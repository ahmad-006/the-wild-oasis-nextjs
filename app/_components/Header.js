    ...
    <header className='relative z-20 bg-transparent sm:px-8 px-4 py-5'>
      <div className='flex justify-between items-center max-w-7xl mx-auto'>
        ...
        <div className="md:hidden">
          <button onClick={() => setIsOpen(!isOpen)} className="text-primary-100 focus:outline-none flex items-center">
            {isOpen ? (
       ...
       >
         <div className="absolute top-8 right-8">
           <button onClick={() => setIsOpen(false)} className="text-primary-100 focus:outline-none hover:text-accent-400 transition-colors">
             <XMarkIcon className="h-8 w-8" />
           </button>
         </div>
